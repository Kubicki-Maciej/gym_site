...
export default function WeightProgressChart({ data }) {
  ...
  // Używamy useMemo do przygotowania danych
  const { chartData, maxSets, hasValidData } = useMemo(() => {
    // Zabezpieczenie - zwracamy puste dane jeśli brak danych wejściowych
    if (!data || data.length === 0) {
      return { chartData: [], maxSets: 0, hasValidData: false };
    }

    let processedChartData = [];
    let processedMaxSets = 0;
    ...
    if (hasSessionStructure) {
      // Format 1: { date, sets: [...] }
      processedMaxSets = Math.max(
        ...data.map(session => session.sets?.length || 0),
      );

      processedChartData = data.map(session => {
        const chartPoint = {
          date: session.date,
        };
      ...
      processedMaxSets = Math.max(
        ...Object.values(groupedByDate).map(sets => sets.length),
      );

      processedChartData = Object.entries(groupedByDate).map(([date, sets]) => {
        ...
        sets.forEach((set, index) => {
          ...
          const oneRM =
            set.oneRM || calculate1RM(set.weight, set.repeats || set.reps || 0);
          if (oneRM > maxOneRM) {
            maxOneRM = oneRM;
          }
        });
    ...
    const isValid = processedChartData.length > 0 && processedMaxSets > 0;

    return {
      chartData: processedChartData,
      maxSets: processedMaxSets,
      hasValidData: isValid,
    };
  }, [data]);

  // Hook useCarousel MUSI być wywołany przed warunkowymi return!
  // Przekazujemy pustą tablicę jeśli nie ma danych
  const {
    visibleItems,
    currentIndex,
    goNext,
    ...
    hasPrev,
    totalItems,
  } = useCarousel(hasValidData ? chartData : [], 6);

  // Teraz możemy mieć warunkowe return
  if (!data || data.length === 0) {
    return (
